...
`monitoramar-compare` roda automaticamente os quatro métodos sobre o mesmo conjunto de
vídeos:
...
4. método completo task-aware (`D_task_aware`).
...
- cobertura de situações raras (nº de contagens de pessoas distintas cobertas, quando
  o YOLOv8 está habilitado; parcela de frames mantidos por evento crítico);
- custo estimado de anotação (heurística: nº de frames × tempo médio por frame).

A tabela `comparison.csv` permite comparar os quatro métodos lado a lado nessas
...
Numa etapa posterior, separada deste repositório: treinar o mesmo YOLOv8/U-Net com
cada um dos quatro datasets gerados na Fase 1 e comparar mAP/precision/recall
(detecção) e mIoU/Dice (segmentação). A divisão treino/validação/teste deve ocorrer
